...
def read_reviews(
    ...
    return crud.get_reviews(
        db, product_id=product_id, status=status, skip=skip, limit=limit
    )
...
def update_review_status(
    review_id: int, status_update: schemas.ReviewUpdate, db: Session = Depends(get_db)
):
    return crud.update_review(db, review_id=review_id, review_update=status_update)
